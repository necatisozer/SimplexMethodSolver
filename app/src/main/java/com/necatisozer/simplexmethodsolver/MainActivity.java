...
public class MainActivity extends AppCompatActivity {
    ...
    private void compute() {
        ...
        int columnSize = equations.get(0).getConstants().size() + 1;

        float[][] standardized = new float[rowSize][columnSize];

        for (int i = 0; i < rowSize; i++) {
            for (int j = 0; j < columnSize - 1; j++) {
                standardized[i][j] = equations.get(i).getConstants().get(j).floatValue();
            }
            standardized[i][columnSize - 1] = equations.get(i).getResult().floatValue();
        }

        Simplex simplex = new Simplex(rowSize - 1, columnSize - 1);

        simplex.fillTable(standardized);
        simplex.setPivotConstantIndexList(pivotConstantIndexList);
        ...
        while (!quit) {
            ...
            if (err == Simplex.ERROR.IS_OPTIMAL) {
                ...
                pivotConstantIndexList = simplex.getPivotConstantIndexList();

                for (int i = 0; i < rowSize - 1; i++) {
                    solution += "x" + (i + 1) + " = ";
                    int index = pivotConstantIndexList.indexOf(i);
                    if (index > -1) {
                        solution += table[index][columnSize - 1];
                    } else {
                        solution += 0;
                    }
                    solution += "\n";
                ...
                solution += "Solution: " + table[rowSize - 1][columnSize - 1];

                quit = true;
            } else if (err == Simplex.ERROR.UNBOUNDED) {
                solution = "Solution is unbounded";
                quit = true;
            }
        }
